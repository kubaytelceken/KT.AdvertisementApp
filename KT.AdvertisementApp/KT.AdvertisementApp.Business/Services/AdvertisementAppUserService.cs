...
namespace KT.AdvertisementApp.Business.Services
{
    public class AdvertisementAppUserService : IAdvertisementAppUserService
    {
        ...
        public async Task<List<AdvertisementAppUserListDto>> GetList(AdvertisementAppUserStatusType type)
        {
            ...
            var list = await query.Include(x => x.Advertisement).Include(x => x.AdvertisementAppUserStatus).Include(x => x.AppUser).ThenInclude(x=>x.Gender).Include(x => x.MilitaryStatus).Where(x=>x.AdvertisementAppUserStatusId == (int)type).ToListAsync();
